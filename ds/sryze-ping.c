...
sryze_ping_context_t *sryze_ping_create(const char *hostname, uint32_t timeout_ms)
{
    ...
    ctx->id = (uint16_t)getpid() + (++next_id);  // Make each context unique
    ctx->seq = 0;
...
bool sryze_ping_send(sryze_ping_context_t *ctx, double *ping_time_ms)
{
    if (!ctx || !ping_time_ms) return false;
